...
* [Microsoft PowerToys](https://docs.microsoft.com/en-us/windows/powertoys/) is a set of utilities (developed by Microsoft) designed to improve the user experience on Windows 10. It has a few key functionalites that are incredibly useful in the lab, namely **Fancy Zones** and **Color Picker**.
...
***Everyone should actively be backing up their data. People have worked hard on projects and lost everything due to hard drive failure. Don't think it wont happen to you - backup your data routinely*** 

There are 3 options for data backup:

1. Ask Nicholas for an external hard drive (HDD) to save data on.

2. Ask Nicholas about SYNC and your ability to store data on the cloud.

3. Once you have completed projects (or are done your time at the IRSS) you must backup your data on the data backup server `\\frst-irsstor\root\IRSSbackup`.
...
It is important to check the `validate` option! Validation take a long time (about the same as the transfer) but gives you a piece of mind that everything is ok. Errors will show up and you can then copy those files again.
